    ...
    if osp.exists(final_info_file) and osp.exists(all_results_file):
        ...
        for dataset in datasets:
            iterations_per_epoch = (
                INFO[dataset]["size"]
                // final_info[dataset]["final_info_dict"]["config"][0]["batch_size"]
            )
            ...
            for seed in seed_numbers:
                ...
                if train_key in results_dict and val_key in results_dict:
                    ...
                    train_iters = [
                        entry["epoch"] * iterations_per_epoch + entry["batch"]
                    ...
                    val_iters = [
                        entry["epoch"] * iterations_per_epoch for entry in val_info
                    ]
            ...
            if all_train_losses:
                ...
                train_losses_array = np.array(
                    [lst[:min_length] for lst in all_train_losses]
                )
            ...
            if all_val_losses:
                ...
                val_losses_array = np.array(
                    [lst[:min_length] for lst in all_val_losses]
                )
