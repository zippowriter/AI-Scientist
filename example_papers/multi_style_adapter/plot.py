...
for i, run in enumerate(runs):
    ...
    for dataset in datasets:
        if "style_consistency_scores" in final_results[run][dataset].get("means", {}):
            means.append(
                final_results[run][dataset]["means"]["style_consistency_scores"].get(
                    "mean_consistency", 0
                )
            )
            stds.append(
                final_results[run][dataset]
                .get("stderrs", {})
                .get("style_consistency_scores", {})
                .get("mean_consistency", 0)
            )
        else:
            means.append(0)
            stds.append(0)
...
for i, run in enumerate(runs):
    ...
    for dataset in datasets:
        if (
            ...
            in final_results[run][dataset]["means"]
        ):
            means.append(
            ...
            stds.append(
                final_results[run][dataset]["stderrs"].get(
                    "avg_inference_tokens_per_second_mean", 0
                )
            )
        else:
            means.append(0)
